...
<form:form commandName="filterMRCForm" method="post" action="${pageContext.request.contextPath}/tb6/index.html">
	...
	<form:form commandName="uploadItem" method="post"
		enctype="multipart/form-data" action="${pageContext.request.contextPath}/tb6/index.html">
		<fieldset>
			...
			<table style="width: 650px">
				<tr>
					...
					<td style="width:450px">
						<div id="btnGroup" >
							<input type="submit" name="btnUpload" value="Upload TB6"
...
<form name="displ" method="post">	
	<sec:authorize access="hasAnyRole('ROLE_CS')">
		...
		<input type="button" value="Send To RMT" onclick="openNotePopup('popup-send-div','popupSendForm')" class="btnSubmit"/>
	</sec:authorize>
		<!--<sec:authorize access="hasAnyRole('ROLE_RM_RMT')">
			<input type="button" value="Receive TB6" onclick="openNotePopup('popup-receive-div','popupForm')" class="btnSubmit"/>
		</sec:authorize>-->
	<br>
	<display:table size="resultSize" export="true" cellpadding="1" cellspacing="1" id="data" name="recordList" class="tftable" requestURI="/tb6/index.html" pagesize="${pageSize}">
		<sec:authorize access="hasAnyRole('ROLE_CS')">
		...
		<display:column property="receiveFromCollectionDate" format="{0,date,dd/MM/yyyy}" title="Receive From Collection Date" />
...
<div id="popup-remark-div" class="hidden dialog" title="Remark TB6">
	<form method="post" id="popupRemarkForm">
		<input type="hidden" name="btnSubmit" value="Remark"/>
		Date Receive From Collection: <input id="dateRemark" type="text" name="dateRemark" size="12" value="" /><br/>
		Remark:<textarea id="remark" rows="5" name="remark" cols="30"></textarea>
	</form>
</div>

<!-- <div id="popup-receive-div" class="hidden dialog" title="Mark as receive TB6">
	<form method="post" id="popupForm">
		<input type="hidden" name="btnSubmit" value="Receive TB6"/>
		Date Receive: <input id="dateReceive" type="text" name="dateReceive" size="12" value="" /><br/>
		Remark:<textarea id="remark" rows="5" name="remark" cols="30"></textarea>
	</form>
</div>
 -->
<div id="popup-send-div" class="hidden dialog" title="Send To RMT">
	<form method="post" id="popupSendForm">
		<input type="hidden" name="btnSubmit" value="Send To RMT"/>
		Date Send: <input id="dateSend" type="text" name="dateSend" size="12" value="" /><br/>
