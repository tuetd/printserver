...
<script type="text/javascript">
	function goViewDetail(id) {
		...
		return openDetailPopup('#dialog-detail-case','detail_followup.html', id,'/csrecord/logs.html');
		//window.location.href = "detail.html?id="+ id;
	}
</script>
<div id="container">
	
	<b>Insurance Certificate Follow Up</b>
	<c:if test="${not empty errorMessage}">
		<center class="errorMessage" >${errorMessage}</center>
	</c:if>
	<form:form commandName="filterRecordForm"  
		method="post" action="${pageContext.request.contextPath}/ic/followup.html">
		<%@ include file="../share/basefilter.jsp"%>
	</form:form>
	
	<sec:authorize access="hasAnyRole('ROLE_CS_RMT')">
	<form:form commandName="uploadItem" method="post"
		enctype="multipart/form-data" action="${pageContext.request.contextPath}/ic/followup.html">
		<fieldset>
	...
	<display:table size="resultSize" export="true"
		...
		class="tftable" requestURI="/ic/followup.html" pagesize="${pageSize}">
		<%@ include file="../share/columns.jsp"%>
	</display:table>
</div>
<%@ include file="detail_followup.jsp"%>