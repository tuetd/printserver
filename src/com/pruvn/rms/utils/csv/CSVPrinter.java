...
public class CSVPrinter implements CSVPrint {
	...
	protected String delimiterString = "#101#";
	 ...
	 * @since ostermillerutils 1.00.00
	 */
	public CSVPrinter(OutputStream out) throws UnsupportedEncodingException{
		this(
	...
	public void writeln(String[] values, boolean isDelimByString) throws IOException {
		try {
			print(values, isDelimByString);
			writeln();
		} catch (IOException iox){
			error = true;
			throw iox;
	...
	public void writeln(String[] values, char specialChar) throws IOException {
		try {
			print(values, specialChar);
			writeln();
		} catch (IOException iox){
			error = true;
			throw iox;
	...
	public void print(String[] values, boolean isDelimByString){
		try {
			write(values, isDelimByString);
		} catch (IOException iox){
			error = true;
		}
	}	
	
	public void print(String[] values, char specialChar){
		try {
			write(values, specialChar);
		} catch (IOException iox){
			error = true;
		}
	}
	...
	public void write(String[] values, boolean isDelimByString) throws IOException {
		try {
			for (int i=0; i<values.length; i++){
				write(values[i], isDelimByString);
			}
		} catch (IOException iox){
			error = true;
			throw iox;
	...
	public void write(String[] values, char specialChar) throws IOException {
		try {
			for (int i=0; i<values.length; i++){
				write(values[i], specialChar);
			}
		} catch (IOException iox){
			error = true;
			throw iox;
	...
	public void writeln(String[][] values, boolean isDelimByString) throws IOException {
		try {
			for (int i=0; i<values.length; i++){
				writeln(values[i], isDelimByString);
			}
			if (values.length == 0){
				writeln();
			}
	...
	public void write(String value, boolean isDelimByString) throws IOException {
		try {
			if (value == null) value = "";
			boolean quote = false;
			if (alwaysQuote){
	...
	public void write(String value, char specialChar) throws IOException {
		try {
			if (value == null) value = "";
			boolean quote = false;
			if (alwaysQuote){
				...
				char c = value.charAt(0);
				if (newLine && (c<'0' || (c>'9' && c<'A') || (c>'Z' && c<'a') || (c>'z')) && c != specialChar){
					quote = true;
				}
				if (c==' ' || c=='\f' || c=='\t'){
					quote = true;
